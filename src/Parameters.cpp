...
// a class with constant coefficients 
...
// a class with constant coefficients 
ParametersFunction::ParametersFunction(std::function<double(double)> f0, long Res0) : f(f0), Res(Res0)
{
}

ParametersInner* ParametersFunction::clone() const
{
	return new ParametersConstant(*this);
}

double ParametersFunction::Integral(double time1, double time2) const
{
	double timeStep = (time2 - time1) / N;
	double sum = 0;
	for (long i = 0; i < res; ++i){
		sum += f((i + 0.5)*timeStep) * timeStep;
	}
	return sum;
}

double ParametersFunction::IntegralSquare(double time1, double time2) const
{
	return (time2 - time1)*ConstantSquare;
}
/*
